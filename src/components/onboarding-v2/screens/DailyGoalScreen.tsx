    ...
    >
      ...
      <div className="max-w-md w-full mx-auto mb-6">
        ...
        <p className="text-sm text-slate-500 mt-2">
          Você decide o ritmo — sem cobrança.
        </p>
      </div>
        ...
        {GOALS.map((goal) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className={`relative w-full flex items-center gap-4 px-4 py-4 rounded-2xl border-2 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 ${
                isSelected
                  ? "border-indigo-500 bg-indigo-50 shadow-sm"
                  : "border-slate-200 bg-white hover:border-indigo-300"
              }`}
            >
              {/* XP grande à esquerda */}
              <div
                className={`flex flex-col items-center justify-center flex-shrink-0 w-16 h-16 rounded-xl ${
                  isSelected
                    ? "bg-gradient-to-br from-indigo-500 to-violet-500 text-white"
                    : "bg-slate-100 text-slate-600"
                }`}
              >
                <span className="text-xl font-bold tabular-nums">{goal.value}</span>
