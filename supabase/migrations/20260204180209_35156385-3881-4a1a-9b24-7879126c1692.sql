-- C05.3 FIX v2: Atualizar canonical_jsonb_string para IGNORAR chaves com valor null
-- Isso espelha o comportamento de canonicalStringify do JavaScript que exclui chaves null
-- CRÍTICO para paridade de hash
...
BEGIN
  ...
  ELSIF json_type = 'object' THEN
    pairs := ARRAY[]::text[];
    -- C05.3 FIX v2: EXCLUIR chaves com valor null (igual ao JavaScript canonicalStringify)
    FOR key, val IN SELECT * FROM jsonb_each(input_jsonb) ORDER BY key
    LOOP
      -- Pular chaves cujo valor é null (jsonb_typeof retorna 'null' para valores JSON null)
...
COMMENT ON FUNCTION public.canonical_jsonb_string(jsonb) IS 'C05.3 v2: Creates canonical JSON string with keys sorted alphabetically, numbers normalized, and null-valued keys EXCLUDED to match JavaScript canonicalStringify for deterministic SHA-256 hashing';