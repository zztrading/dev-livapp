...
BEGIN
  ...
  -- For lesson_completed re-completions: allow with 25% XP
  IF v_event_exists AND p_event_type = 'lesson_completed' THEN
    v_is_review := true;
    -- Don't block, continue with reduced XP below
  ELSIF v_event_exists THEN
    -- All other event types: strict idempotency (return 0)
  ...
  IF p_event_type = 'lesson_completed' THEN
    ...
    -- Apply 25% reduction for re-completions
    IF v_is_review THEN
      v_xp_delta := GREATEST(1, ROUND(v_xp_delta * 0.25));
      v_coins_delta := GREATEST(0, ROUND(v_coins_delta * 0.25));
    END IF;
  ...
  -- Record event (for reviews, use NULL reference_id to bypass unique constraint)
  INSERT INTO public.user_gamification_events (
    user_id, event_type, event_reference_id, payload, xp_delta, coins_delta
  ) VALUES (
    v_user_id, p_event_type,
    CASE WHEN v_is_review THEN NULL ELSE p_event_reference_id END,
    p_payload, v_xp_delta, v_coins_delta
  );
